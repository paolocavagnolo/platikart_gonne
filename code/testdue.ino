...
void loop() {
 ...
 if (analogRead(switchPinA) < 500) {
  ...
  accendi();
  if (analogRead(switchPinB) < 500) {
    if (ret) {
      incrA = veloStat;
    }
    prime = true;
    veloc = incrA;
    analogWrite(velPin, veloc)
    ...
    ret = true;
    freq = map(incrA, 0, massimo, 1500, 250);
    period = 3.1415*millis()/freq;
    veloSin = veloStat + int(incrB*sin(period));
    analogWrite(velPin, veloSin);
    Serial.println(veloSin);
  }
 }
